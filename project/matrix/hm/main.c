// https://github.com/HuMeng11/matrix
...
void QR(Matrix* A, Matrix* Q, Matrix* R) {
    ...
    for (int j = 0; j < A->column; j++) {
        ...
        for (int k = 0; k < j; k++)  //计算第j列以前
        {
            ...
            for (int i1 = 0; i1 < col_A.row;
                 i1++) {  // R=Q'A(Q'即Q的转置) 即Q的第k列和A的第j列做内积
                R->data[k * R->column + j] +=
                    col_A.data[i1] * Q->data[i1 * Q->column + k];  // Q的第k列
            }
            for (int i2 = 0; i2 < A->column; i2++) {
                col_Q.data[i2] -=
                    R->data[k * R->column + j] * Q->data[i2 * Q->column + k];
            }
        }
    ...
    for (int i = 0; i < A->row; i++) {
        for (int j = 0; j < B->column; j++) {
            for (int k = 0; k < A->column; k++)
                C->data[i * C->row + j] +=
                    A->data[i * A->row + k] * B->data[k * B->column + j];
        }
    }
...
void Eig(Matrix* A, Matrix* eigenVector, Matrix* eigenValue) {
    ...
    for (int count = 0; count < num; count++) {
        ...
        CopyMatrix(
            A,
            &temp);  //这个每次都要重新复制，因为后面会破坏原矩阵(刚开始没注意到这个找bug找了好久。。)
        ...
        for (int i = 0; i < temp.row - 1; i++) {
            ...
            for (int i1 = i + 1; i1 < temp.row; i1++) {
                ...
                for (int j1 = i; j1 < temp.column; j1++) {
                    temp.data[i1 * temp.column + j1] -=
                        coe * temp.data[i * temp.column + j1];
                }
            }
        }
        //让最后一行为1
...
int main() {
    const unsigned NUM = 5;  //最大迭代次数，让数据更准确
    ...
    while (1)  // VS里面运行写scanf会报错，改成scanf_s就可以了
    {
        ...
        int num = row * col;
        printf("按照以行的输入顺序依次输入矩阵内的元素，一共输入%d个元素：",
               num);
        int data;
        for (int i = 0; i < num; i++) {
            scanf("%d", &data);
